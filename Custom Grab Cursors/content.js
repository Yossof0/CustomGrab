(async function () {
  console.log(
    "The Custom Grab Cursor Extension Was Programmed And Developed By Yossof0, If you are willing to share, Please give credits to the owner",
  );
  ...
  if (!stored.packs && (stored.grabCursor || stored.grabbingCursor)) {
    packs[DEFAULT_PACK_ID] = {
      ...
      grabbingCursor:
        stored.grabbingCursor || packs[DEFAULT_PACK_ID].grabbingCursor,
      syncHotspot: stored.syncHotspot !== false,
      hotspotX:
        typeof stored.hotspotX === "number"
          ? stored.hotspotX
          : packs[DEFAULT_PACK_ID].hotspotX,
      hotspotY:
        typeof stored.hotspotY === "number"
          ? stored.hotspotY
          : packs[DEFAULT_PACK_ID].hotspotY,
      hotspotGrabX:
        typeof stored.hotspotGrabX === "number"
          ? stored.hotspotGrabX
          : packs[DEFAULT_PACK_ID].hotspotGrabX,
      hotspotGrabY:
        typeof stored.hotspotGrabY === "number"
          ? stored.hotspotGrabY
          : packs[DEFAULT_PACK_ID].hotspotGrabY,
      hotspotGrabbingX:
        typeof stored.hotspotGrabbingX === "number"
          ? stored.hotspotGrabbingX
          : packs[DEFAULT_PACK_ID].hotspotGrabbingX,
      hotspotGrabbingY:
        typeof stored.hotspotGrabbingY === "number"
          ? stored.hotspotGrabbingY
          : packs[DEFAULT_PACK_ID].hotspotGrabbingY,
    };
  }

  const activePackId =
    (REMOVED_PACKS.includes(stored.activePackId)
      ? DEFAULT_PACK_ID
      : stored.activePackId) || DEFAULT_PACK_ID;
  ...
  const HOTSPOT_X = syncHotspot
    ? (pack.hotspotX ?? 16)
    : (pack.hotspotGrabX ?? 16);
  const HOTSPOT_Y = syncHotspot
    ? (pack.hotspotY ?? 16)
    : (pack.hotspotGrabY ?? 16);
  ...
  chrome.storage.onChanged.addListener((changes, area) => {
    ...
    if (
      ...
      changes.siteRules ||
      changes.cursorTrail ||
      changes.cursorTrailColor ||
      changes.cursorTrailSize ||
      changes.cursorTrailLength ||
      changes.cursorTrailFade ||
      changes.cursorTrailOpacity ||
      changes.cursorOpacity ||
      changes.cursorTintColor ||
      changes.cursorTintStrength
    ) {
  ...
  const cursorOpacity =
    typeof stored.cursorOpacity === "number" ? stored.cursorOpacity : 1;
  const cursorTintColor = stored.cursorTintColor || "#ffffff";
  const cursorTintStrength =
    typeof stored.cursorTintStrength === "number"
      ? stored.cursorTintStrength
      : 0;
  ...
  async function processImageUrl(extUrl) {
    ...
    try {
      ...
      return await new Promise((res) =>
        canvas.toBlob(
          (blob) => res(blob ? URL.createObjectURL(blob) : extUrl),
          "image/png",
        ),
      );
    } catch (e) {
  ...
  const trailBaseOp =
    typeof stored.cursorTrailOpacity === "number"
      ? stored.cursorTrailOpacity
      : 0.9;
    ...
    if (trailScope === "grab-only") {
      document.addEventListener(
        ...
        (e) => {
          ...
          if (!el) return;
          const isGrab =
            el.getAttribute("data-grab-ext") != null ||
    ...
    if (!window.__grabExtensionGlobalListenerAttached) {
      const globalMouseUp = () => {
        ...
        document.querySelectorAll("*").forEach((el) => {
          ...
          if (
            computed === "grab" ||
            computed === "-webkit-grab" ||
            (computed.includes("url") && computed.includes("grab"))
          ) {
  ...
  const observer = new MutationObserver((mutationsList) => {
    for (const mutation of mutationsList) {
      if (mutation.type === "childList") {
        ...
        if (
          ["style", "class", "draggable", "data-grab"].includes(
